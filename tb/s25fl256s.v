...
specify
    ...
    `else
        ...
        specparam        tdevice_PRGSUSP           = 40e6;//
        // VCC (min) to CS# Low
   specparam        tdevice_PU                     = 100;//3e8;//tPU
        // PPB Erase Time
        specparam        tdevice_PPBERASE          = 15e9;//
        // Password Unlock Time
        specparam        tdevice_PASSULCK          = 1e6;//
 ...
 reg deq_holdin;
   ...
   initial begin
    ...
    begin: InitMemory
       ...
       status=$value$plusargs("SPIFLASH_IMAGE=%s",image);
       if(!status) begin
          $display("%m: SPIFLASH_IMAGE plusarg not defined, not loading SPI flash.");
       end
       else begin
          $display("%m: SPI flash model loading %-s into array.", image);
          $readmemh(image, Mem);
       end
       /*
   ...
   begin: Buscycle
        ...
        begin
            ...
            begin                    // at SI are latched on the rising edge of SCK
                ...
                if (CHECK_FREQ)
                begin
                    if ((CLK_PER < 20 && Latency_code == 3) ||
                    (CLK_PER < 12 && Latency_code == 0) ||
                    (CLK_PER < 11 && Latency_code == 1) ||
                    (CLK_PER < 9 && Latency_code == 2))
                    begin
...
///////////////////////////////////////////////////////////////////////////////
    ...
    begin
        ...
        begin
            RST = 1'b1;
            #200ns RST = 1'b0;  // 200 ns
        end
        else if (Instruct == RESET)
        begin
            Reseted = 1'b0;
            #10ns Reseted = 1'b1;   //  10 ns
        end
    end
...
//    /////////////////////////////////////////////////////////////////////////
   ...
   always @(rising_edge_PoweredUp or falling_edge_write or
            ...
            begin
                ...
                begin
                    ...
                    begin
                        ...
                        RES_TO_SUSP_MIN_TIME <= #60ns 1'b0;//60 ns
                        RES_TO_SUSP_TYP_TIME = 1'b1;
                        RES_TO_SUSP_TYP_TIME <= #100000ns 1'b0;//100us
                    end
    ...
    begin
        ...
        #1ns rising_edge_read_out = 1'b0;
    end

    always @(negedge write)
    begin
        falling_edge_write = 1;
        #1ns falling_edge_write = 0;
    end

    always @(posedge PRGSUSP_out)
    begin
        PRGSUSP_out_event = 1;
        #1ns PRGSUSP_out_event = 0;
    end

    always @(posedge ERSSUSP_out)
    begin
        ERSSUSP_out_event = 1;
        #1ns ERSSUSP_out_event = 0;
    end
    ...
    begin
        ...
        #50ns falling_edge_RSTNeg = 1'b0;
    end

    always @(posedge RSTNeg_in)
    begin
        rising_edge_RSTNeg = 1'b1;
        #10ns rising_edge_RSTNeg = 1'b0;
    end

    always @(negedge RST)
    begin
        falling_edge_RST = 1'b1;
        #10ns falling_edge_RST = 1'b0;
    end

    always @(posedge RST)
    begin
        rising_edge_RST = 1'b1;
        #1ns rising_edge_RST = 1'b0;
    end
